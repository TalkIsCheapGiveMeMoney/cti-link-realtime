...
import com.tinet.ctilink.realtime.inc.IvrStaticParameters;
...
public class GetIvrOptionServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		jsonObject.put("__" + IvrStaticParameters.CDR_CUSTOMER_NUMBER, caller.getCallerNumber()); //客户号码
		jsonObject.put("__" + IvrStaticParameters.CDR_CUSTOMER_NUMBER_TYPE, caller.getTelType()); //电话类型
		jsonObject.put("__" + IvrStaticParameters.CDR_CUSTOMER_AREA_CODE, areaCode); //区号
		...
		if(trunk != null){
			jsonObject.put(IvrStaticParameters.NUMBER_TRUNK_AREA_CODE, trunk.getAreaCode());
		}
		...
		if(enterpriseId == null){
			jsonObject.put(IvrStaticParameters.ENTERPRISE_STATUS, Const.ENTITY_STATUS_NO_SERVICE);//enterprise_status 企业目前无中继状态
		}else{
			//把企业id设置到通道里面，在ivrNode和黑名称接口中有用到
			jsonObject.put("__" + IvrStaticParameters.ENTERPRISE_ID, String.valueOf(enterpriseId));
			...
			if( enterpriseHotline != null && StringUtil.isNotEmpty(enterpriseHotline.getHotline())){
				...
				jsonObject.put(IvrStaticParameters.CDR_HOTLINE, enterpriseHotline.getHotline());
			}else{
				if(trunk != null){
					jsonObject.put(IvrStaticParameters.CDR_HOTLINE, trunk.getAreaCode() + trunk.getNumberTrunk());
				}
			}
			...
			if (entity != null) {
				...
				if (enterpriseStatus == Const.ENTITY_STATUS_OK) {//0:未开通 1:正常 2:欠费 3:停机 4:注销
					...
					jsonObject.put(IvrStaticParameters.ENTERPRISE_STATUS, enterpriseStatus);//企业状态
					...
					if (s!=null && s.getName().equals(Const.ENTERPRISE_SETTING_NAME_RESTRICT_TEL_TYPE) && (s.getValue().equals("1") || s.getValue().equals("2"))) {//开启了黑白名单功能
						jsonObject.put(IvrStaticParameters.IS_RESTRICT_CHECK, "1");
					}
					s = redisService.get(Const.REDIS_DB_CONF_INDEX, String.format(CacheKey.ENTERPRISE_SETTING_ENTERPRISE_ID_NAME, enterpriseId
					...
					if (s != null && s.getName().equals(Const.ENTERPRISE_SETTING_NAME_IS_RECORD) && s.getValue().equals("1")) { //开启IVR录音功能
						jsonObject.put(IvrStaticParameters.IS_RECORD, "1");
						jsonObject.put(IvrStaticParameters.RECORD_SCOPE, s.getProperty());
					}
					s = redisService.get(Const.REDIS_DB_CONF_INDEX, String.format(CacheKey.ENTERPRISE_SETTING_ENTERPRISE_ID_NAME, enterpriseId
					...
					if (s != null && s.getName().equals(Const.ENTERPRISE_SETTING_NAME_IS_CRBT_OPEN) && s.getValue().equals("1")) {  //开启彩铃, 呼入不自动answer
						jsonObject.put(IvrStaticParameters.IS_CRBT_OPEN, "1");
					}

					if(callType.equals(String.valueOf(Const.CDR_CALL_TYPE_OB_WEBCALL))){

						if(StringUtil.isNotEmpty(webcallIvrId)){
							jsonObject.put("__" + IvrStaticParameters.IVR_ID, webcallIvrId);
							jsonObject.put(IvrStaticParameters.VALID_IVR, "1");
						}else{
							s = redisService.get(Const.REDIS_DB_CONF_INDEX, String.format(CacheKey.ENTERPRISE_SETTING_ENTERPRISE_ID_NAME, enterpriseId
									, Const.ENTERPRISE_SETTING_NAME_WEBCALL_DEFAULT_IVR), EnterpriseSetting.class);
							jsonObject.put("__" + IvrStaticParameters.IVR_ID, s.getValue());
							jsonObject.put(IvrStaticParameters.VALID_IVR, "1");
						}
					}else{//按照呼入检查
								...
								if ((accordingWeek || accordingSpecialDate) || StringUtil.isEmpty(ruleTime)) {
									if (areaBl || StringUtil.isEmpty(ruleAreaNumber)) {
										if (trunkBl || StringUtil.isEmpty(ruleTrunkNumber)) {
											jsonObject.put("" + IvrStaticParameters.IVR_ROUTER_TYPE, ruleType);
											switch (ruleType) {
												case 1://******************1:IVR*****************************//*

													jsonObject.put("__" + IvrStaticParameters.IVR_ID, ruleProperty);
													...
													if (enterpriseIvr == null) {
														jsonObject.put(IvrStaticParameters.VALID_IVR, "0");
													} else {
														jsonObject.put(IvrStaticParameters.VALID_IVR, "1");
													}
													found = true;
													break;
												case 2://*****************2:固定电话 **************************//*
													jsonObject.put(IvrStaticParameters.IVR_ROUTER_TEL, ruleProperty);
													found = true;
													break;
												case 3://*****************3:分机*******************************//*
													jsonObject.put(IvrStaticParameters.IVR_ROUTER_EXTEN, ruleProperty);
													found = true;
													break;
											}
										}
					...
					jsonObject.put(IvrStaticParameters.ENTERPRISE_STATUS, enterpriseStatus);
				}
			} else {
